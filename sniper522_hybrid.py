# =====================================================================================
# SNIPER PHOENIX v5.2.2 - DETECTOR HÍBRIDO INTELIGENTE
#
# Base: v5.2.1 (flags de filtro independentes M1/M2/M3)
#
# [v5.2.2] Correções no Detector Híbrido:
#
#   BUG 1 — Spam de log (arquivo de 7MB):
#     • Deduplicação por minuto: cada tipo de anomalia (PUMP/CRASH) é detectado
#       no máximo UMA VEZ por minuto-calendário no RT detector.
#     • Cooldown de log em trade_ws_loop: mínimo 60s entre logs WARNING do mesmo
#       tipo, independente de quantos ticks disparem a condição.
#     • Gate de confiança mínima no log: anomalias com confidence < 15% não geram
#       WARNING (são silenciosas — ainda armazenadas para o HybridDetector avaliar).
#
#   BUG 2 — Falsos positivos com volume abaixo da média (Vol: 0.4×):
#     • Condição anterior: volume_ok OR velocity_extreme
#     • Condição nova: volume_ok OR (velocity_extreme AND volume_ratio >= 1.0)
#     • Velocidade extrema sozinha com volume abaixo da média não é suficiente.
#
#   BUG 3 — Confidence = 0% mesmo com volume alto (Vol: 12.3×, Vel: 0.000%/s):
#     • A fórmula anterior incluía `abs(velocity)/VELOCITY_THRESHOLD` no min(),
#       zerando a confidence quando velocity ≈ 0, mesmo com spike e volume altos.
#     • Corrigido: velocidade só entra no cálculo se for realmente extrema (≥ threshold).
#
#   BUG 4 — Threshold RT-only cancelado pelos pesos (partial sell nunca disparava):
#     • Antes: `score_rt >= MIN_CONF_SELL * WEIGHT_RT`
#       onde score_rt = confidence * WEIGHT_RT → equivalia a confidence >= MIN_CONF_SELL
#       (pesos se cancelavam, RT tinha o mesmo bar que 1h → partial sell impossível)
#     • Depois: `rt_pump['confidence'] >= MIN_CONF_SELL * WEIGHT_RT`
#       Threshold efetivo para RT-only = 0.70 × 0.6 = 0.42 (vs 0.70 para 1h) ✓
#
#   BUG 5 — AnomalyDetector 1h recebia vela aberta (incompleta):
#     • update() e detect_flash_pump/crash agora recebem a penúltima vela (fechada).
#     • O detector RT já cobre a vela em andamento via WebSocket.
#
#   MELHORIA — Log de diagnóstico quando anomalia detectada mas score insuficiente:
#     • Registra INFO (não WARNING) com scores e threshold para facilitar calibração.
#
# [v5.2.1] Flags de ativação independente por filtro:
#   • filtro_rsi_ativo, filtro_volume_ativo, circuit_breaker_ativo
#
...
class RealtimeAnomalyDetector:
    ...
    def __init__(self, symbol="PAXGUSDT", baseline_minutes=60,
        ...
        # [v5.2.2] Deduplicação por minuto — cada tipo dispara no máximo 1× por minuto
    ...
    def _detect_realtime_anomaly(self, velocity):
        """
        [v5.2.2] Correções:
          1. Deduplicação por minuto: cada tipo dispara no máximo 1× por minuto.
          2. Gate de volume: velocidade extrema só é aceita se volume >= média (1×).
          3. Confidence: velocidade só entra no min() se for realmente extrema.
        """
        ...
        try:
            ...
            # [v5.2.2] Gate de volume: velocidade extrema sozinha só aceita com volume >= média
            ...
            def calcular_confidence(spike_ou_drop: float) -> float:
                """
                [v5.2.2] Velocidade só entra no min() se for extrema,
                evitando que velocity=0 zere a confidence com spike+volume altos.
                """
            ...
            if crash_drop >= self.PRICE_SPIKE_THRESHOLD and (volume_ok or velocity_ok):
                # [v5.2.2] Deduplicação: ignora se já detectou crash neste minuto
            ...
            if pump_spike >= self.PRICE_SPIKE_THRESHOLD and (volume_ok or velocity_ok):
                # [v5.2.2] Deduplicação: ignora se já detectou pump neste minuto
...
class HybridDetector:
    ...
    [v5.2.2] Correção do threshold RT-only:
      Antes: score_rt >= MIN_CONF_SELL * WEIGHT_RT
             onde score_rt = confidence * WEIGHT_RT
             → equivalia a confidence >= MIN_CONF_SELL (pesos se cancelavam)
             → RT tinha o mesmo bar que 1h → partial sell era praticamente impossível

      Depois: rt_pump['confidence'] >= MIN_CONF_SELL * WEIGHT_RT
              Threshold efetivo RT-only = 0.70 × 0.6 = 0.42 (vs 0.70 para 1h) ✓

    [v5.2.2] Log de diagnóstico quando anomalia detectada mas score insuficiente.
    """

    WEIGHT_RT = 0.6
    WEIGHT_1H = 1.0

    # [v5.2.2] Confiança mínima para gerar log de diagnóstico (não dispara ação, mas registra)
    MIN_CONF_DIAG = 0.20
    ...
    def evaluate_pump(self, candle_1h_data, profit_pct, current_price):
        ...
        # [v5.2.2] FIX: compara confiança bruta com threshold reduzido
        # Threshold RT-only efetivo = MIN_CONF_SELL * WEIGHT_RT = 0.42
        # (antes os pesos se cancelavam, exigindo confiança >= MIN_CONF_SELL = 0.70)
        ...
        # [v5.2.2] Diagnóstico: anomalia detectada mas score insuficiente para agir
    ...
    def evaluate_crash(self, candle_1h_data, in_position, capital_available):
        ...
        # [v5.2.2] Diagnóstico: crash detectado mas score insuficiente
        if (rt_crash or crash_1h) and best_score >= self.MIN_CONF_DIAG:
            logging.info(
                f"[HYBRID DIAG CRASH] Detectado mas score insuficiente — "
                f"RT={score_rt:.2f} | 1h={score_1h:.2f} | threshold={self.MIN_CONF_BUY:.2f}")
...
CONFIG = {
    ...
    # [v5.2.2] Confiança mínima para gerar WARNING no log (abaixo = silencioso)
    "RT_LOG_MIN_CONFIDENCE": 0.15,
    # [v5.2.2] Cooldown de log por tipo (segundos entre WARNINGs do mesmo tipo)
...
def carregar_configuracoes():
    ...
    Auditoria.log_sistema(
        ...
        f"| Timeout: {CONFIG['HYBRID_PARTIAL_TIMEOUT_SEC']}s "
        f"| Log conf mín: {CONFIG['RT_LOG_MIN_CONFIDENCE']:.0%} "
        f"| Log cooldown: {CONFIG['RT_LOG_COOLDOWN_SEC']}s", "INFO")
...
def loop_dca_ativo():
    ...
    while True:
        ...
        try:
            # ------------------------------------------------------------------
            # [v5.2.2] Obtém vela FECHADA (iloc[-2]) para o AnomalyDetector 1h.
            # A vela aberta atual (iloc[-1]) é monitorada pelo RT detector via WS.
            # Requer limit >= 3 para garantir ao menos 2 velas fechadas.
            # ------------------------------------------------------------------
            df_1h_atual    = obter_df_1h(limit=25)
            candle_fechada = None   # vela fechada — usada pelo AnomalyDetector 1h
            ...
            if df_1h_atual is not None and len(df_1h_atual) > 1:
                ...
                # update() sempre recebe a penúltima vela (fechada e consolidada)
                anomaly_detector.update(candle_fechada)
            ...
            # candle_fechada: vela 1h fechada passada ao AnomalyDetector
            # ------------------------------------------------------------------
            if hybrid_detector and hybrid_detector.ENABLED and candle_fechada is not None:
        ...
        try:
            ...
            if not em_op:
                ...
                if hybrid_detector and hybrid_detector.ENABLED and preco > 0:
                    # [v5.2.2] Usa vela FECHADA (iloc[-2]) para o AnomalyDetector 1h
...
# =============================================================================
# [HIBRIDO] THREAD TRADE WEBSOCKET
# =============================================================================
async def trade_ws_loop():
    ...
    [v5.2.2] Controle de spam de log:
      • Gate de confidence: anomalias com confidence < RT_LOG_MIN_CONFIDENCE
        são silenciosas no log (mas ainda armazenadas para o HybridDetector).
      • Cooldown por tipo: mínimo RT_LOG_COOLDOWN_SEC segundos entre WARNINGs
        do mesmo tipo (PUMP ou CRASH) — evita inundar o log com o mesmo evento.
    """
    ultimo_symbol = CONFIG['SYMBOL'].replace('/', '').lower()
    uri = f"wss://stream.binance.com:9443/ws/{ultimo_symbol}@trade"

    # [v5.2.2] Controle de cooldown de log por tipo
                ...
                async for message in ws:
                    ...
                    if realtime_detector and realtime_detector.ENABLED:
                        ...
                        if anomaly:
                            ...
                            conf_min    = CONFIG["RT_LOG_MIN_CONFIDENCE"]
                            cooldown    = CONFIG["RT_LOG_COOLDOWN_SEC"]
                            agora       = datetime.now()
                            ultimo_log  = _ultimo_log.get(tipo)

                            # [v5.2.2] Só loga se:
                            #   1. Confidence acima do mínimo configurado
                            #   2. Cooldown expirado para este tipo
                            deve_logar = (
                                confidence >= conf_min and
                                (ultimo_log is None or
                                 (agora - ultimo_log).total_seconds() >= cooldown)
                            )
...
def thread_visual():
    while True:
        if not menu_ativo:
            ...
            c = s["preco"]
            print(f"{Fore.CYAN}🐦‍🔥 SNIPER PHOENIX v5.2.2 {Fore.WHITE}| DETECTOR HÍBRIDO | {Fore.CYAN}UPTIME: {uptime}")
