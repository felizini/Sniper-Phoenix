# =====================================================================================
# SNIPER PHOENIX v5.2.4 - DETECTOR HÍBRIDO INTELIGENTE
#
# Base: v5.2.3
#
# [v5.2.4] Melhorias de robustez e observabilidade:
#
#   MELHORIA-01 — Versionamento de schema no sniper_state.json
#     • Campo "schema_version" adicionado ao arquivo de estado (valor atual: 2).
#     • carregar_estado_disco() detecta versões antigas (sem o campo ou version < 2)
#       e emite AVISO no log descrevendo quais campos estarão ausentes/com default.
#     • Nunca rejeita o arquivo — migração é silenciosa e segura.
#     • Histórico: v1 = formato original (sem partial sell); v2 = com partial sell.
#
#   MELHORIA-02 — RotatingFileHandler para ARQUIVO_LOG_SISTEMA
#     • Substitui FileHandler pelo RotatingFileHandler do Python.
#     • Limite: 5 MB por arquivo, máximo 5 backups (total ~25 MB).
#     • Evita crescimento ilimitado como o arquivo de 7 MB causado pelo BUG 1 v5.2.2.
#     • Configurável via config.ini: [logging] max_bytes e backup_count.
#
#   MELHORIA-03 — Log de diagnóstico de confiança apenas quando thresholds atingidos
#     • Antes: [HYBRID DIAG PUMP/CRASH] era logado quando score >= MIN_CONF_DIAG (20%)
#       mas ABAIXO dos thresholds de ação — útil para calibração, porém verboso.
#     • Agora: o log INFO de diagnóstico só é emitido quando a confiança RT ou 1h
#       atinge ou supera o threshold configurado (emergency_sell/buy_min_confidence),
#       mas o score combinado ainda não foi suficiente para disparar ação.
#     • Anomalias abaixo do threshold de confiança mínima são completamente silenciosas.
#
...
class HybridDetector:
    ...
    def evaluate_pump(self, candle_1h_data, profit_pct, current_price):
        ...
        # [MELHORIA-03] Log de diagnostico emitido somente quando a confianca de ao menos
        # um detector atinge o respectivo threshold de acao (sell). Anomalias fracas
        # (abaixo do threshold) sao silenciosas — evita poluicao no log.
        rt_atingiu_threshold = rt_pump  and rt_pump['confidence']  >= (self.MIN_CONF_SELL * self.WEIGHT_RT)
        h1_atingiu_threshold = pump_1h and pump_1h['confidence'] >= self.MIN_CONF_SELL
        if (rt_atingiu_threshold or h1_atingiu_threshold):
            logging.info(
                f"[HYBRID DIAG PUMP] Detectado mas score combinado insuficiente — "
    ...
    def evaluate_crash(self, candle_1h_data, in_position, capital_available):
        ...
        # [MELHORIA-03] Diagnostico apenas quando ao menos um detector atingiu
        # o proprio threshold de acao (buy). Abaixo disso: silencioso.
        rt_atingiu_threshold  = rt_crash  and rt_crash['confidence']  >= (self.MIN_CONF_BUY * self.WEIGHT_RT)
        h1_atingiu_threshold  = crash_1h and crash_1h['confidence'] >= self.MIN_CONF_BUY
        if rt_atingiu_threshold or h1_atingiu_threshold:
            logging.info(
                f"[HYBRID DIAG CRASH] Detectado mas score combinado insuficiente — "
...
class Auditoria:
    @staticmethod
    def configurar():
        # [MELHORIA-02] RotatingFileHandler: limita crescimento do log em disco.
        # Limite: LOG_MAX_BYTES por arquivo, LOG_BACKUP_COUNT backups (~25 MB total).
        # basicConfig() nao suporta rotating, entao configuramos manualmente.
...
CONFIG = {
    ...
    # [MELHORIA-02] Rotacao de log
    "LOG_MAX_BYTES":    5_242_880,  # 5 MB por arquivo
    "LOG_BACKUP_COUNT": 5,          # ate 5 backups -> ~25 MB total
}
...
# [MELHORIA-01] Versionamento de schema do sniper_state.json.
# Incrementar sempre que novos campos obrigatórios forem adicionados.
#   v1 — formato original (sem partial sell)
#   v2 — adicionados partial_sell_active/qtd_restante/custo_restante  [v5.2.3]
STATE_SCHEMA_VERSION = 2


def salvar_estado_disco():
    """
    [FIX-09]      Persiste estado de partial sell para sobreviver a reinicializações.
    [MELHORIA-01] Inclui schema_version para detectar arquivos de versões anteriores.
    """
    try:
        with state_lock:
            partial_active  = hybrid_detector.partial_sell_active if hybrid_detector else False
            dados = {
                "schema_version": STATE_SCHEMA_VERSION,          # [MELHORIA-01]
...
def carregar_estado_disco():
    ...
    [MELHORIA-01] Detecta e adverte sobre arquivos de schema anterior.
                  A migração é sempre segura — campos ausentes recebem defaults.
    """
    ...
    try:
        ...
        if dados.get("em_operacao") and dados.get("symbol") == CONFIG["SYMBOL"]:
            ...
            # [MELHORIA-01] Verifica schema e emite aviso descritivo se desatualizado
            version_salva = dados.get("schema_version", 1)
            if version_salva < STATE_SCHEMA_VERSION:
                campos_ausentes = []
                if version_salva < 2:
                    campos_ausentes += [
                        "partial_sell_active (default=False)",
                        "partial_sell_qtd_restante (default=0.0)",
                        "partial_sell_custo_restante (default=0.0)",
                    ]
                Auditoria.log_sistema(
                    f"AVISO DE SCHEMA: sniper_state.json v{version_salva} < "
                    f"v{STATE_SCHEMA_VERSION} esperado. "
                    f"Campos com default: {', '.join(campos_ausentes)}. "
                    f"Migração automática aplicada — nenhuma ação necessária.", "AVISO")
            ...
            Auditoria.log_sistema(
                f"ESTADO RESTAURADO: {len(ordens_executadas)} ordens "
                f"(schema v{version_salva}).", "AVISO")
            return True
    except Exception as e:
    ...
    if os.path.exists("config.ini"):
        ...
        try:
            ...
            with config_lock:
                ...
                if "logging" in cp:
                    gl = cp["logging"].get
                    # [MELHORIA-02] Permite ajustar rotacao via config.ini sem recompilar
                    CONFIG["LOG_MAX_BYTES"]    = int(gl("log_max_bytes",    CONFIG["LOG_MAX_BYTES"]))
                    CONFIG["LOG_BACKUP_COUNT"] = int(gl("log_backup_count", CONFIG["LOG_BACKUP_COUNT"]))
