# =====================================================================================
# SNIPER PHOENIX v5.2.0 - DETECTOR HÍBRIDO INTELIGENTE
#
# Base: v5.1.0 (com todas as melhorias M1-M6)
#
# [HIBRIDO] Novo: Detector Híbrido Inteligente
...
def loop_dca_ativo():
    ...
    partial_sell_qtd_restante   = 0.0   # qtd ainda em carteira após venda parcial
    partial_sell_custo_restante = 0.0   # custo proporcional do restante
    ...
    while True:
        ...
        try:
            ...
            # Se venda parcial foi feita, usa qtd/custo do restante
            ...
            #    Bloqueado durante venda parcial ativa (hybrid aguardando confirmação)
            # ------------------------------------------------------------------
            ...
            #
            #  Fluxo:
            #   A) Nenhuma venda parcial ativa:
            #      • SELL_ALL      → vende tudo, encerra ciclo
            #      • SELL_PARTIAL  → vende CONFIG[PARTIAL_RATIO]%, aguarda confirmação
            #   B) Venda parcial ativa:
            #      • HybridDetector.evaluate_pump retorna SELL_ALL_REMAINING
            #        (quando 1h confirma OU timeout expira)
            #        → vende qtd restante, encerra ciclo
            # ------------------------------------------------------------------
            if hybrid_detector and hybrid_detector.ENABLED and candle_atual is not None:
                ...
                if action == "SELL_ALL":
                    ...
                    definir_status(
                        f"✅ HYBRID VENDA RESTANTE! [{source}] "
                        f"Score: {score:.0%}", "SUCESSO")
            ...
            #    Só ativo se não houver venda parcial pendente
            # ------------------------------------------------------------------
        ...
        try:
            ...
            async with websockets.connect(uri, ping_interval=20, ping_timeout=10) as ws:
                ...
                async for message in ws:
                    ...
                    # Alimenta o detector em tempo real (thread-safe por deque + float)
...
def thread_visual():
    while True:
        if not menu_ativo:
            ...
            c = s["preco"]
            print(f"{Fore.CYAN}🐦‍🔥 SNIPER PHOENIX v5.2.0 {Fore.WHITE}| DETECTOR HÍBRIDO | {Fore.CYAN}UPTIME: {uptime}")
